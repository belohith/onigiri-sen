...
const badgeImages: Record<string, { en: string; ja: string }> = {
  GF:         { en: "/images/badges/gluten_free.png",  ja: "/images/badges/gluten_free_jp.png" },
  Vegan:      { en: "/images/badges/vegan.png",        ja: "/images/badges/vegan_jp.png" },
  Vegetarian: { en: "/images/badges/vegetarian.png",   ja: "/images/badges/vegetarian_jp.png" },
  Organic:    { en: "/images/badges/organic.png",      ja: "/images/badges/organic_jp.png" },
};
  ...
  return (
    <div style={{ position: "relative", paddingTop: isMobile ? 40 : 60 }}>
      ...
      <div style={{
        ...
        width: mascotSize, height: mascotSize, borderRadius: "50%", background: "#fff",
        boxShadow: "0 2px 10px rgba(0,0,0,0.10)", display: "flex",
      ...
      <div style={{ background: "#fff", borderRadius: 20, boxShadow: "0 2px 16px rgba(0,0,0,0.08)", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{
          ...
          minHeight: isMobile ? 180 : 280, position: "relative", background: "#fff",
        }}>
          <img
            src={f.img}
          ...
          {!f.comingSoon && (
            <div style={{ display: "flex", gap: isMobile ? 4 : 8, marginBottom: isMobile ? 6 : 10, flexWrap: "wrap" as const, minHeight: isMobile ? 34 : 44 }}>
              {f.tags.map((tag) => (
                <img
                  key={tag}
                  src={badgeImages[tag]?.[lang as "en" | "ja"] ?? badgeImages[tag]?.en}
                  alt={tag}
                  style={{ width: isMobile ? 34 : 44, height: isMobile ? 34 : 44, objectFit: "contain", flexShrink: 0 }}
                />
              ))}
            </div>
          )}
  ...
  return (
    <section style={{ background: "#fff9f5" }}>
      ...
      <div style={{ background: "#ffefc8", padding: isMobile ? "20px 16px 40px" : "28px 48px 52px" }}>
        <h2 style={{ textAlign: "center", fontWeight: 900, fontSize: isMobile ? 18 : 25, letterSpacing: 3, color: "#6f471c", margin: "0 0 20px", textTransform: "uppercase" as const }}>
          {t("OUR FLAVORS", "フレーバー")}
        </h2>

        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(3, 1fr)", gap: isMobile ? 10 : 14, maxWidth: isMobile ? "100%" : 800, margin: "0 auto" }}>
          {flavors.map((f) => (
