  ...
  return (
    <>
      ...
      <main style={{ fontFamily:"DM Sans, sans-serif", background:"#fff" }}>
        ...
        <section style={{ padding:"92px 80px 48px", textAlign:"center", background:"#fff9f5" }}>
          <h1 style={{ fontWeight:800, fontSize:38, color:"#6f471c", margin:"0 0 16px" }}>{t("Get in Touch","お問い合わせ")}</h1>
          <p style={{ color:"#6f471c", fontSize:16, maxWidth:500, margin:"0 auto" }}>
        ...
        <section style={{ background:"#ffefc8", padding:"64px 80px", display:"grid", gridTemplateColumns:"1fr 1.8fr", gap:56, alignItems:"start" }}>
          <div>
            <h2 style={{ fontWeight:900, fontSize:28, color:"#6f471c", margin:"0 0 12px" }}>{t("Contact Info","連絡先情報")}</h2>
            
            {[
              { icon:"✉", label:t("E-mail","メール"), value:"contact@onigirisen.jp", href:"mailto:order@onigirisen.com" },
              ...
              <div key={row.label} style={{ display:"flex", alignItems:"center", gap:18, marginBottom:28 }}>
                ...
                <div>
                  ...
                  <a href={row.href} style={{ color:"#6f471c", fontWeight:600, fontSize:15, textDecoration:"none" }}>{row.value}</a>
                </div>
              </div>
            ))}

            <div style={{ background:"#6f471c", borderRadius:18, padding:"22px 24px", marginTop:40, color:"#fff" }}>
          ...
          <div style={{ background:"#fff9f5", borderRadius:24, padding:"40px", boxShadow:"0 4px 28px rgba(0,0,0,0.08)" }}>
            <h2 style={{ fontWeight:800, fontSize:22, color:"#6f471c", margin:"0 0 28px" }}>{t("Send Us a Message","メッセージを送る")}</h2>
            ...
            <div style={{ marginBottom:24 }}>
              <div style={{ fontWeight:700, fontSize:14, color:"#6f471c", marginBottom:6 }}>{t("Message *","メッセージ *")}</div>
              <textarea placeholder={t("How can we help?","どのようにお手伝いできますか？")} rows={5} style={{ ...input, resize:"vertical" } as React.CSSProperties} />
            </div>
            <Link href="/thank-you" style={{ display:"block", background:"#e8847a", color:"#fff", padding:"15px", borderRadius:999, fontFamily:"DM Sans, sans-serif", fontWeight:700, fontSize:16, textAlign:"center", textDecoration:"none", marginBottom:14 }}>
