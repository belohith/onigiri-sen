...
const pressRelease = {
  ...
  outlet: "Onigiri Sen",
  titleEn: "Press Release — Onigiri Sen Now Available at Draeger's Market Menlo Park",
  titleJa: "プレスリリース — Onigiri Sen、Draeger's Market メンロパーク店に新登場",
  descEn: "Official press release for the Onigiri Sen launch at Draeger's Market Menlo Park. 5 flavors available daily — made fresh every morning.",
  descJa: "Draeger's Market メンロパーク店でのOnigiri Sen出店に関する公式プレスリリース。5種類のフレーバーを毎朝新鮮にお届けします。",
  date: "6 August 2026",
  href: "/press/draegers-menlo-park",
};

const sanjosePressRelease = {
  category: "print" as const,
  articleLang: "both" as const,
  outlet: "Onigiri Sen",
      ...
      <main style={{ fontFamily:"DM Sans, sans-serif", background:"#fff9f5" }}>
        ...
        <section style={{ padding: isMobile ? "0 16px 64px" : "0 80px 100px" }}>
          <div style={{ display:"flex", flexDirection:"column" as const, gap:12 }}>
            ...
            {/* ── SAN JOSE T&T PRESS RELEASE ── */}
            {(() => {
              const item = sanjosePressRelease;
              const cat = categoryLabel[item.category];
              return (
                <Link key="sanjose-press-release" href={item.href} style={{ textDecoration: "none" }}>
                  <div
                    style={{
                      background: "#fffbf0",
                      borderRadius: 20,
                    ...
                    onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 20px rgba(237,126,128,0.18)"; }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = "none"; }}
                  >
                    {isMobile ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" as const }}>
                        <div style={{ fontWeight: 800, fontSize: 13, color: "#6f471c" }}>{item.outlet}</div>
                        <div style={{ background: cat.bg, color: cat.color, fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "2px 8px" }}>{lang === "ja" ? cat.ja : cat.en}</div>
                        <div style={{ fontSize: 11, color: "#bbb", fontWeight: 500 }}>{item.date}</div>
                        <div style={{ background: langLabel[item.articleLang].bg, color: langLabel[item.articleLang].color, fontSize: 9, fontWeight: 700, borderRadius: 999, padding: "2px 8px" }}>{langLabel[item.articleLang].label}</div>
                      </div>
                    ) : (
                      <div style={{ textAlign: "center" as const, minWidth: 100 }}>
                        <div style={{ fontWeight: 800, fontSize: 14, color: "#6f471c", marginBottom: 6, lineHeight: 1.3 }}>{item.outlet}</div>
                        <div style={{ fontSize: 11, color: "#bbb", fontWeight: 500 }}>{item.date}</div>
                        <div style={{ display: "inline-block", marginTop: 8, background: cat.bg, color: cat.color, fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "3px 10px" }}>{lang === "ja" ? cat.ja : cat.en}</div>
                        <div style={{ display: "inline-block", marginTop: 6, background: langLabel[item.articleLang].bg, color: langLabel[item.articleLang].color, fontSize: 9, fontWeight: 700, borderRadius: 999, padding: "2px 8px" }}>{langLabel[item.articleLang].label}</div>
                      </div>
                    )}
                    <div>
                      <div style={{ fontWeight: 800, fontSize: isMobile ? 14 : 16, color: "#2a2a2a", marginBottom: 6, lineHeight: 1.4 }}>{lang === "ja" ? item.titleJa : item.titleEn}</div>
                      <p style={{ color: "#8a6a4a", fontSize: 13, lineHeight: 1.75, margin: 0 }}>{lang === "ja" ? item.descJa : item.descEn}</p>
                    </div>
                    <div style={{ color: "#ed7e80", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center" }}><ExternalIcon /></div>
                  </div>
                </Link>
              );
            })()}
