...
function ImgBox({ src, alt, style }: { src: string; alt: string; style: React.CSSProperties }) {
  return (
    <div style={{ background:"#c8bfb5", overflow:"hidden", position:"relative", ...style }}>
...
/>
      ...
      <button onClick={next} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"rgba(255,255,255,0.8)", border:"none", borderRadius:"50%", width:40, height:40, cursor:"pointer", fontSize:22, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", color:"#6f471c" }}>
        ›
      </button>
  ...
  return (
    <>
      ...
      <main style={{ fontFamily:"DM Sans, sans-serif", background:"#fff9f5" }}>
        ...
        {/* ── PAGE TITLE ── */}
        <section style={{ padding:"56px 80px 0", textAlign:"center", background:"#fff9f5" }}>
          <h1 style={{ fontWeight:800, fontSize:28, color:"#6f471c", margin:0 }}>
            {t("Our Story","私たちのストーリー")}
          </h1>
        </section>

        {/* ── MISSION ── */}
        <section style={{ position:"relative", minHeight:340, overflow:"hidden", marginTop:32, display:"flex", alignItems:"center" }}>
          <div style={{ position:"absolute", inset:0 }}>
            <img src="/images/story-mission.png" alt="" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover" }} />
          </div>
          <div style={{ position:"relative", zIndex:1, width:"100%", textAlign:"center", padding:"64px 120px", color:"#fff" }}>
            <h2 style={{ fontWeight:900, fontSize:38, lineHeight:1.2, margin:"0 0 20px" }}>
              {t("Mission\nBringing Onigiri into American Daily Life.", "ミッション\nおにぎりをアメリカの日常へ")}
            </h2>
            <p style={{ maxWidth:600, margin:"0 auto", fontSize:15, lineHeight:1.85, opacity:0.9 }}>
              {t(
        ...
        {/* ── FOUNDER'S STORY ── */}
        <section style={{ background:"#fff8f4", padding:"48px", margin:"48px 0", overflow:"hidden" }}>
          <div style={{ position:"relative", maxWidth:1500, margin:"0 auto" }}>
            ...
            <div style={{ background:"#f3a8b6", width:"72%", padding:"52px 200px 52px 80px", position:"relative", zIndex:10 }}>
              <h2 style={{ fontWeight:900, fontSize:40, color:"#fff", margin:"0 0 32px", fontStyle:"italic" }}>
        ...
        {/* ── 1000 YEARS ── */}
        <section style={{ position:"relative", overflow:"hidden", color:"#fff", minHeight:600 }}>
          <img src="/images/story-1000y.png" alt="1000 years story" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover" }} />
          <div style={{ position:"relative", zIndex:1, textAlign:"center", padding:"80px 120px" }}>
            <h2 style={{ fontWeight:900, fontSize:36, lineHeight:1.25, margin:"0 0 28px" }}>
              {t("A 1,000-Year Tradition for the Next 1,000 Years","1,000年の伝統を次の1,000年へ")}
            </h2>
            <div style={{ border:"1px solid rgba(255,255,255,0.45)", borderRadius:12, padding:"24px 36px", maxWidth:560, margin:"0 auto" }}>
              <p style={{ opacity:0.9, fontSize:14, lineHeight:1.9, margin:0 }}>
                {t(
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:48 }}>
            <div>
              {[
                ...
                <div key={item.date} style={{ display:"grid", gridTemplateColumns:"76px 20px 1fr", gap:"0 14px", marginBottom:i<2?40:0 }}>
                  <div style={{ fontWeight:600, fontSize:13, color:"#6f471c", paddingTop:2, textAlign:"right" as const }}>{item.date}</div>
                  ...
                  <div>
                    <div style={{ fontWeight:800, fontSize:16, color:"#6f471c", marginBottom:6, lineHeight:1.3 }}>{item.title}</div>
        ...
        <section style={{ padding:"80px 80px", background:"#fff9f5" }}>
          <div style={{ textAlign:"center", marginBottom:48 }}>
            ...
            <h2 style={{ fontWeight:800, fontSize:28, color:"#6f471c", margin:"0 0 10px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24 }}>
            {[
              ...
              <div key={item.brand} style={{ background:"#faf6f0", borderRadius:20, overflow:"hidden" }}>
                <div style={{ height:300, background:"#c8bfb5", overflow:"hidden" }}>
        ...
        {/* ── INGREDIENTS ── */}
        <section style={{ padding:"0 80px 80px", background:"#ffefc8" }}>
          <div style={{ textAlign:"center", padding:"64px 0 48px" }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"#ED7E80", borderRadius:999, padding:"7px 20px", fontSize:11, fontWeight:700, letterSpacing:1.5, color:"#fff", marginBottom:20, textTransform:"uppercase" as const }}>
              {t("INGREDIENTS","素材")}
            </div>
            <h2 style={{ fontWeight:800, fontSize:28, color:"#6f471c", margin:0 }}>
            ...
            {[
              ...
              <div key={item.label} style={{ background:"#fff", borderRadius:16, overflow:"hidden" }}>
                ...
                <div style={{ padding:"18px 20px 24px" }}>
                  <div style={{ fontWeight:800, fontSize:12, color:"#6f471c", marginBottom:8, textTransform:"uppercase" as const, letterSpacing:0.5, lineHeight:1.4 }}>{item.label}</div>
